...
abstract class CameraActivity : BaseActivity(), ICameraStateCallBack {
    ...
    override fun initView() {
        when (val cameraView = getCameraView()) {
            ...
            getCameraViewContainer()?.apply {
                removeAllViews()
                addView(view as View, getViewLayoutParams(this))
            }
        }
    }
    ...
    protected fun openCamera(st: IAspectRatio? = null) {
        when (st) {
            is TextureView, is SurfaceView -> st
            else -> null
        }.apply {
    ...
    private fun getViewLayoutParams(viewGroup: ViewGroup): ViewGroup.LayoutParams {
        return when (viewGroup) {
            ...
            is RelativeLayout -> {
                ...
                "Unsupported container view, you can use FrameLayout or LinearLayout or RelativeLayout"
            )
        }
    }
     ...
     *
     * @return camera view container, such as FrameLayout etc
     */
